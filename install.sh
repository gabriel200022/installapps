...
install_pritunl() {
  ...
  local home_dir app_name script_name bundle_identifier pritunl_url
  home_dir="$(get_home_dir)"
  ...
  echo "Fetching dynamic URL for latest Pritunl release..."
...
If no app is provided, installs all apps in sequential order.
Allowed values:
  brew
  nvm
  telegram
