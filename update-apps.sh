...
uninstall_xcode_clt() {
  echo "🧹 Eliminare Xcode Command Line Tools..."
  run_cmd_as_root rm -rf /Library/Developer/CommandLineTools || true
  echo "✅ Xcode CLT a fost șters."
}
...
uninstall_brew() {
  ...
  echo "✅ Homebrew a fost eliminat complet."
}
...
usage() {
  cat <<EOF
Usage: $0 [argument]
Install values: clt, brew, nvm, telegram, googledrive, compass, postman, vscode, iterm2, chrome, pritunl, docker
Uninstall values: uninstall, uninstall-clt, uninstall-chrome, etc.
Reinstall values: reinstall, reinstall-clt, reinstall-chrome, etc.
Update values: update, update-clt, update-chrome, etc.
EOF
}
...
run_app() {
  case "$1" in
    ...
    uninstall-clt)       uninstall_xcode_clt ;;
    ...
    reinstall-clt)       uninstall_xcode_clt && install_xcode_clt ;;
