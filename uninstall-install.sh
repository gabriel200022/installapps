...
# --- METODE INSTALARE (Cele pe care le avem deja salvate) ---
...
install_pritunl() {
  ...
  if [ -e "$process/Contents/Info.plist" ]; then
    ...
    local locations=("$home_dir/Library" "$home_dir/Library/Application Support" "/Library/Application Support" "/Library/LaunchDaemons")
    for location in "${locations[@]}"; do
      [[ -d "$location" ]] || continue
      find "$location" -maxdepth 1 -iname "*$app_name*" -prune -print 2>/dev/null | while IFS= read -r p; do [ -n "$p" ] && run_cmd_as_root rm -rf "$p" 2>/dev/null || true; done
    done
  fi
...
uninstall_chrome() {
  ...
  # Curățare date reziduale și profile cache
  run_cmd_as_root rm -rf "/Library/Application Support/Google/Chrome" || true
  run_cmd_as_root rm -rf "/Library/Google/GoogleSoftwareUpdate" || true
  ...
  echo "✅ Google Chrome a fost șters complet."
}
...
uninstall_telegram() {
  ...
  echo "✅ Telegram a fost șters complet."
}
...
uninstall_google_drive() {
  ...
  echo "✅ Google Drive a fost șters."
}
...
uninstall_compass() {
  ...
  echo "✅ MongoDB Compass a fost șters."
}
...
uninstall_docker() {
  ...
  echo "✅ Docker Desktop a fost șters."
}
...
uninstall_pritunl() {
  ...
  # Logica ta de deep cleanup pe profile
  ...
  # Curățare directoare conexe din locations
  ...
  echo "✅ Pritunl și profilele asociate au fost șterse complet."
}
...
uninstall_all() {
  ...
  uninstall_nvm
  uninstall_brew
  echo "🏁 Toate aplicațiile din listă au fost șterse din sistem!"
}
...
usage() {
  cat <<EOF
Usage: $0 [app/uninstall-app]
Allowed install values: telegram, googledrive, compass, postman, vscode, iterm2, chrome, pritunl, docker, brew, nvm
Allowed uninstall values: uninstall, uninstall-chrome, uninstall-telegram, uninstall-vscode, etc.
EOF
}

run_app() {
  case "$1" in
    # Mapare procese instalare
    brew) install_brew ;;
    nvm) install_nvm ;;
    ...
    # Mapare procese dezinstalare (Individuale + Global)
